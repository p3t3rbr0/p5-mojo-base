...
sub show_migrations {
    ...
    if (my @migrations = @{$app->model('Migration')->find([qw/name applied_at comment/])}) {
        ...
        for (@migrations) {
            _print(undef, '+++', 'green');
            _print(undef, " $_->{name}\t$_->{applied_at}\t$_->{comment}\n", 'white');
        }
        _print(undef, ('-' x 80) . "\n\n", 'white');
    } else {
        _print(undef, "--- empty ---\n\n", 'white');
...
sub upgrade_db {
    ...
    unless (@$files) {
        _print(undef, "--- Migrations diretory is empty ---\n", 'white');
        exit;
    }
    ...
    unless (@$new_migrations) {
        _print(undef, "--- No new migrations found ---\n", 'white');
        exit;
    }

    # Применяем новые миграции
    for my $migration (sort { $a cmp $b } @$new_migrations) {
        _print('applying', "$migration... ", 'white');
        ...
        unless ($comment) {
            _print(undef, "FAILED\n\n", 'red');
            _print(undef, "Comment is required attribute!\n", 'white');
            exit 1;
        }
        ...
        unless ($apply_script) {
            _print(undef, "FAILED\n\n", 'red');
            _print(undef, "Apply sql-script is required!\n", 'white');
            exit 1;
        }
        ...
        unless ($revert_script) {
            _print(undef, "FAILED\n\n", 'red');
            _print(undef, "Revert sql-script is required!\n", 'white');
            exit 1;
        }

        $app->model('Migration')->begin();
...
sub downgrade_db {
    ...
    unless ($migrations) {
        _print(undef, "--- No migrations for revert ---\n", 'white');
        exit;
    }

    for my $migration (@$migrations) {
        _print('reverting', "$migration->{name}\t$migration->{applied_at}\t$migration->{comment}... ", 'white');
